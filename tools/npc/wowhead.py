...
class WowheadNPC(NPC):
    ...
    def __info(self):
        if self._info is False:
            ...
            info = re.search(
                r"\$\.extend\(g_npcs\[%d\], ({.+?})\);" % self.id, self.__page()
            )
            if info:
    ...
    def _name(self):
        info = re.search(
            r"g_pageInfo = {type: 1, typeId: \d+, name: \"(.+?)\"};", self.__page()
        )
        if info:
            return self.html_decode(
                info.group(1).replace("\\'", "'").replace('\\"', '"')
            )
    ...
    def _locations(self):
        ...
        for zone, data in alldata.items():
            ...
            coords[zoneid_to_mapid[zone]] = [
                pack_coords(c[0], c[1]) for c in processed_coords
            ]
        return coords
    ...
    def _quest(self):
        ...
        if search:
            return search
        info = re.search(
            r'<pre id="questtracking">/run print\(IsQuestFlaggedCompleted\((\d+)\)\)</pre>',
            self.__page(),
        )
        if info:
    ...
    def _expansion(self):
        patch = re.search(
            r"Added(?:\s|\\x20)in(?:\s|\\x20)patch(?:\s|\\x20)(\d+)", self.__page()
        )
        if patch:
    ...
    @classmethod
    def query(
        cls, category, expansion, session, ptr=False, beta=False, cached=True, **kw
    ):
        url = "%s/%s-npcs/classification:4:2?filter=39;%d;0" % (
        ...
        npcs = {}
        for npc in (
            WowheadNPC(id, ptr=ptr, session=session)
            for id in re.findall(r'"id":(\d+)', match.group(1))
        ):
            try:
                print(npc)
                npcs[npc.id] = npc
...
class WowHeadQuestSearch:
    ...
    def __call__(self, name, session):
        if not self.quests:
            ...
            match = re.search(
                r"^new Listview\({.+?id:\s*'quests',.*?data:\s*(.+)}\);$",
                questpage,
                re.MULTILINE,
            )
            self.quests = [
                (int(q["id"]), q["name"]) for q in yaml.safe_load(match.group(1))
            ]
            self.quests.sort()
